...
#iptables -N LOG_DROP
#iptables -A LOG_DROP -m limit --limit 5/min -j LOG
#iptables -A LOG_DROP -j DROP

#iptables -A FORWARD -s 192.168.100.0/24 -d 172.20.0.0/24 -j LOG_DROP
#iptables -A FORWARD -s 172.20.0.0/24 -d 192.168.100.0/24 -j LOG_DROP

# 1. Create the custom chain
#iptables -N LOG_DROP

# 2. Add the logging and dropping behavior to that chain
#iptables -A LOG_DROP -j LOG 
#iptables -A LOG_DROP -j DROP

# 3. Direct the traffic into the now-existing chain
#iptables -A FORWARD -s 192.168.100.0/24 -d 172.20.0.0/24 -j LOG_DROP
#iptables -A FORWARD -s 172.20.0.0/24 -d 192.168.100.0/24 -j LOG_DROP
# Save rules
